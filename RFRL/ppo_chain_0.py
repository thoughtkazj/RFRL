...
class BetaActor(nn.Module):  # Beta演员
	def __init__(self, state_dim, action_dim, net_width):  # （状态维度，动作维度，网络宽度）
		super(BetaActor, self).__init__() # super继承父类nn.Module
...
class GaussianActor_musigma(nn.Module): # 高斯演员_musigma
...
class GaussianActor_mu(nn.Module):  #高斯演员_mu
...
class Critic(nn.Module):  # 评论员
	...
	def forward(self, state):
		# print("self.C1", self.C1.weight)
		# print("self.C2", self.C2)
		# print("self.C3", self.C3)
...
class PPO(object):
	...
	def select_action(self, state):#only used when interact with the env 仅在与环境交互时使用
		with torch.no_grad():
			state = state.astype(float)
			state = torch.FloatTensor(state.reshape(1, -1)).to(device)
			dist = self.actor.get_dist(state)
			a = dist.sample()
			a = torch.clamp(a, 0, 1)  # 限制a在（0，1）之间
			logprob_a = dist.log_prob(a).cpu().numpy().flatten()
			return a.cpu().numpy().flatten(), logprob_a

	def evaluate(self, state):#only used when evaluate the policy仅在评估策略时使用.Making the performance more stable
		with torch.no_grad():
			state = state.astype(float)
			state = torch.FloatTensor(state.reshape(1, -1)).to(device)
			if self.dist == 'Beta':
	...
	def train(self):
		...
		# print("self.critic1_yuan", self.critic.C1.weight)

		# print("self.critic1_gai", self.critic.C1.weight)
		s, a, r, s_prime, logprob_a, done_mask, dw_mask = self.make_batch()
		# print("self.critic1", self.critic.C1.weight)

		''' Use TD+GAE+LongTrajectory to compute Advantage and TD target'''
		"""时间差分+泛化优势估计+长轨迹去计算优势和差分目标"""
		with torch.no_grad():
			vs = self.critic(s)
			...
			# adv = adv.numpy()
			# # save
			# np.savetxt("D:/22_env.tens2/EV2/battery4/AC_PPO2_FRL/MG1_ADV.csv", adv.reshape((1, -1)), delimiter=",",
			# 		   fmt="%7f")
			#
			# ppo_yue2.main2(self.ep1, self.bian)
			# self.ep1 = self.ep1 + 1
			# # get
			# adv = np.loadtxt("D:/22_env.tens2/EV2/battery4/AC_PPO2_FRL/AVG_ADV.csv", delimiter=",",
			# 				 dtype=float).reshape(
			# 	(-1, 1))
		...
		for i in range(self.K_epochs):
			...
			#Shuffle 洗牌 the trajectory, Good for training
	...
	def save(self,episode):
		torch.save(self.critic.state_dict(), "./model1/ppo_critic{}.pth".format(episode))
		torch.save(self.actor.state_dict(), "./model1/ppo_actor{}.pth".format(episode))


	def load(self,episode):
		self.critic.load_state_dict(torch.load("./model1/ppo_critic{}.pth".format(episode)))
		self.actor.load_state_dict(torch.load("./model1/ppo_actor{}.pth".format(episode)))
